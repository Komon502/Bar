...
if($_SERVER['REQUEST_METHOD'] == 'POST' && !$hasConcert){
    ...
    if(in_array($table_name, $reserved_tables)){
        echo "<script>alert('ขออภัย โต๊ะนี้เพิ่งถูกจองไป'); window.location.reload();</script>"; exit();
    }
...
<html lang="th">
<head>
    <meta charset="UTF-8"><title>จองโต๊ะร้านอาหาร</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        .container { padding-top:30px; padding-bottom:100px; }
        .date-selector { background:#fff; padding:20px; border-radius:15px; box-shadow:0 5px 20px rgba(0,0,0,0.05); text-align:center; margin-bottom:30px; }
        .date-input { padding:10px 20px; font-size:1.2rem; border:2px solid #ddd; border-radius:50px; outline:none; color:#333; }
        
        /* Concert Alert Card */
        .concert-alert { background: linear-gradient(135deg, #ff9a9e 0%, #fecfef 99%, #fecfef 100%); padding:40px; border-radius:20px; text-align:center; color:#c0392b; box-shadow:0 10px 30px rgba(255, 154, 158, 0.3); }
        .btn-buy-ticket { background:#c0392b; color:white; padding:12px 30px; border-radius:50px; text-decoration:none; font-weight:bold; display:inline-block; margin-top:20px; box-shadow:0 5px 15px rgba(192, 57, 43, 0.3); transition:0.3s; }
        .btn-buy-ticket:hover { transform:translateY(-3px); background:#a93226; }

        /* Seat Map (Reuse Style) */
        .seat-map-wrapper { background:#2c3e50; padding:40px; border-radius:15px; text-align:center; overflow-x:auto; }
        .screen { background:#34495e; color:#fff; padding:5px 20px; border-radius:20px; display:inline-block; margin-bottom:30px; font-size:0.8rem; }
        .seat-grid { display:inline-grid; grid-gap:10px; justify-content:center; }
        .seat-item { width:45px; height:45px; border-radius:8px; background:#fff; display:flex; align-items:center; justify-content:center; font-size:0.8rem; cursor:pointer; font-weight:bold; transition:0.2s; border-bottom:4px solid #ddd; }
        
        /* Status */
        .seat-item.available:hover { transform:translateY(-3px); background:#3498db; color:white; border-bottom-color:#2980b9; }
        .seat-item.selected { background:#2ecc71; color:white; border-bottom-color:#27ae60; box-shadow:0 0 15px #2ecc71; }
        .seat-item.booked { background:#555; color:#888; border-bottom-color:#333; cursor:not-allowed; }
        .seat-item.vip { border:2px solid #f1c40f; }

        /* Checkout Form */
        .booking-form { max-width:600px; margin:30px auto; background:#fff; padding:30px; border-radius:15px; box-shadow:0 5px 20px rgba(0,0,0,0.05); display:none; }
    ...
    <div class="container">
        ...
        <div class="date-selector">
            <form method="get">
                ...
                <input type="date" name="date" class="date-input" value="<?= $date ?>" onchange="this.form.submit()" min="<?= date('Y-m-d') ?>">
            </form>
        </div>
            ...
            <div class="seat-map-wrapper">
                ...
                <div class="seat-grid" style="grid-template-columns: repeat(<?= $max_col ?>, 45px);">
                    ...
                    for($r = 1; $r <= $max_row; $r++){
                        for($c = 1; $c <= $max_col; $c++){
                            $found = null;
                            foreach($tables as $t){ if($t['row_idx']==$r && $t['col_idx']==$c) $found=$t; }
            ...
            <form method="post" class="booking-form" id="bookingForm">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div class="input-group">
                        ...
                        <select name="guests">
                            <?php for($i=1; $i<=10; $i++) echo "<option value='$i'>$i ท่าน</option>"; ?>
                ...
                <button type="submit" class="btn-main" style="width:100%; background:#2c3e50; color:#fff; padding:12px; border:none; border-radius:8px; font-size:1.1rem; cursor:pointer;">
                    ยืนยันการจอง
                </button>
            </form>
        <?php endif; ?>
